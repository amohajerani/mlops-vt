...
def parse_args():
    ...
    parser.add_argument(
        "--traffic_allocation",
        type=str,
        help="Deployment traffic allocation percentage",
    )
    parser.add_argument(
        "--scoring_script_dir", type=str, help="Directory of the scoring script"
    )
    parser.add_argument(
        "--resource_limit_cpu",
        type=str,
        help="The CPU resource settings for a container.",
    )
    parser.add_argument(
        "--resource_limit_memory",
        type=str,
        help="The memory resource settings for a container.",
    )
...
def main():
    ...
    code_configuration = CodeConfiguration(
        code=args.scoring_script_dir, scoring_script="score.py"
    )
    ...
    online_deployment = KubernetesOnlineDeployment(
        ...
        environment_variables={"DEPLOYMENT_NAME": args.deployment_name},
    )
    ...
    endpoint_update_job = ml_client.online_endpoints.begin_create_or_update(
        online_endpoint
    )
