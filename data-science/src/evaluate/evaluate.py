...
def bias_testing(protected_groups, X, y, yhat, evaluation_output):
    ...
    with open((Path(evaluation_output) / "bias_results.txt"), "w") as outfile:
        for group, results in bias_results.items():
            ...
            outfile.write(f"  p-value abs: {results['p_value_abs']:.2f}\n")
            outfile.write(f"  p-value se: {results['p_value_se']:.2f}\n")
            outfile.write(f"  Confidence interval abs: {results['ci_abs']:.2f}\n")
            outfile.write(f"  Confidence interval se: {results['ci_se']:.2f}\n")
